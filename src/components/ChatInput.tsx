...
interface ChatInputProps {
  ...
  isListening: boolean;
  transcript: string;
  inputValue?: string; // Make inputValue optional
...
export const ChatInput: React.FC<ChatInputProps> = ({ onSend, onStartVoice, onStopVoice, isListening, transcript, inputValue = '', setInputValue }) => {
  const [input, setInput] = useState(inputValue); // Initialize with inputValue
  ...
  return (
    <form onSubmit={handleSubmit} className="flex items-center gap-2 p-4 border-t bg-white">
      <button
        ...
        className={`p-2 rounded-full ${isListening ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-600'} hover:bg-gray-200 transition-colors`}
      >
        <Mic size={20} />
      </button>
      <input
